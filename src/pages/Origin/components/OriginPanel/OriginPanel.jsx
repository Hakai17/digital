...
import { fieldsOrigin } from "../../constants";
...
async function setDefaultUserValues({
  ...
  try {
    openBackdrop();
    const data = await get(`/cadastroAtendimento/buscarOrigem?id=${originId}`);
    fieldsOrigin.forEach(field => setFieldValue(field, data[field], false));
...
export function OriginPanel({ isAddMode, originId, ...props }) {
