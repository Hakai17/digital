...
import { Indicator3DataForm } from "./components/Indicator3DataForm";
import { SideBarIndicator3 } from "./components/SideBarIndicator3";
...
export function PagesIndicator3({ onSave = null }) {
  const [id, setId] = useState("");
  const [descricao, setDescricao] = useState("");
  const [selectedIndicator, setSelectedIndicator] = useState({});
  const [editarAdicionar, setEditarAdicionar] = useState(false);
  const [open, setOpen] = useState(false);
  const { data: indicators3, refetch } = useQuery({
    queryKey: ["indicators3", { id, descricao }],
    queryFn: () => post("/produto/listagemId3", { id, descricao }),
  });
